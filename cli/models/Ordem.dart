...
class ItemPeca {
  ...
  @override
  String toString() {
    final nome = peca?.marca ?? 'peça #$pecaId';
    final valor = valorUnitario != null ? valorUnitario!.toStringAsFixed(2) : '-';
    return '  - $quantidade x $nome (R\$$valor cada)';
  }
}
...
class ItemServico {
  ...
  @override
  String toString() {
    final nome = servico?.nome ?? 'serviço #$servicoId';
    final valor = valorUnitario != null ? valorUnitario!.toStringAsFixed(2) : '-';
    return '  - $nome (R\$$valor)';
  }
}
...
class Ordem {
  ...
  @override
  String toString() {
    final placa = veiculo?.placa ?? 'veículo #$veiculoId';
    final abertura = dataHoraAbertura?.toLocal().toString() ?? '-';
    final conclusao = dataHoraConclusao?.toLocal().toString();
    final buffer = StringBuffer(
      '#$id  $placa  Status: $status  Total: R\$${valorTotal.toStringAsFixed(2)}  Aberta em: $abertura',
    );
    if (conclusao != null) buffer.write('  Concluída em: $conclusao');
